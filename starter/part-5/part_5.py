...
def dictionaries_list(book_source):
    books_list = []
    with open(book_source, "r") as f:
        file = f.readlines()
        for line in file:
...
def view_books(book_source):
    print("\nHere are all your books...\n")
    for book in dictionaries_list(book_source):
        get_book_printed(book)

def get_highest_rated(book_source):
    list = dictionaries_list(book_source)
...
def get_sorted_by_rating(book_source):
...
Choose 3 to see a total count of your books
Choose 4 to see the amount of pages in your books
Choose 5 to see the your highest rated book
Choose 6 to see your lowest rated book
Choose 7 to see your books ranked
Choose 0 to exit
Type here: """)
        ...
        if choice == "1":
            ...
            print(f"\nYou have a total of {len(dictionaries_list(book_source))} books.\n")
        elif choice == "4":
            print(f"\nYou books page count totals {sum([x['pages'] for x in dictionaries_list(book_source)])} pages!\n")
        elif choice == "5":
            print("\nHere is your highest rated book...\n")
            get_book_printed(get_highest_rated(book_source))
        elif choice == "6":
            print("\nHere is your lowest rated book...\n")
            get_book_printed(get_lowest_rated_book(book_source))
        elif choice == "7":
            get_sorted_by_rating(book_source)
        elif choice == "0":
            print("\nGoodbye")
            active = False
        else:
